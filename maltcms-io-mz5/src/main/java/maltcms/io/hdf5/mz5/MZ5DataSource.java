...
//            log.debug("{} has no source files!", f.getParent().getUri());
...
//                file = FileTools.getFile(ff).getAbsolutePath();
    ...
    public ArrayList<Array> readIndexed2(final IVariableFragment f)
        ...
        Array index_array_raw = readSingle(index);
        switch (DataType.getType(index_array_raw.getElementType())) {
            case LONG:
        ...
        Array index_array = Array.factory(DataType.getType(index_array_raw.getElementType()), index_array_raw.getShape());
        int scans = index_array.getShape()[0];
        switch (DataType.getType(index_array_raw.getElementType())) {
            case LONG:
                index_array.setLong(0, 0L);
                break;
            case INT:
                index_array.setInt(0, 0);
                break;
        }
        for (int i = 1; i < scans; i++) {
            switch (DataType.getType(index_array_raw.getElementType())) {
                case LONG:
                    index_array.setLong(i, index_array_raw.getLong(i - 1));
                    break;
                case INT:
                    index_array.setInt(i, index_array_raw.getInt(i - 1));
                    break;
            }
        }
        log.info("index array: {}", index_array);
