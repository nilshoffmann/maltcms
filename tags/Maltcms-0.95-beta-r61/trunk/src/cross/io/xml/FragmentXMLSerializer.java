...
public class FragmentXMLSerializer implements IDataSource {
	...
	public IFileFragment deserialize(String location) throws IOException {
	...
	protected IFileFragment handleFile(final Element root) {
		...
		final IFileFragment ff1 = FileFragmentFactory.getInstance()
		        .getFragment(f.getAbsolutePath(), null);
		handleAttributes(ff1, file);
		final List<?> l = file.getChildren("namedGroup");
		HashSet<String> idxVars = new HashSet<String>();
	...
	protected IVariableFragment handleVariable(final IFileFragment parent,
	        final Element var) {
		String varname = var.getAttribute("name").getValue();
		if (parent.hasChild(varname)) {
			return parent.getChild(varname);
		}
		IVariableFragment vf = parseVariable(parent, var);
		return vf;
	}
	...
	protected IVariableFragment parseVariable(final IFileFragment parent,
		...
		Dimension[] dims = parseVariableDimensions(var);
		...
		for (final Object o : l) {
			...
			if (range != null) {
				...
				try {
					r = new Range(r_first.getIntValue(), r_last.getIntValue(),
					        r_stride.getIntValue());
				} catch (final DataConversionException e) {
					this.log.error(e.getLocalizedMessage());
	 ...
	 * @return
	 */
	private Array handleData(final String name, Dimension[] dims,
	        final Element data) {
		EvalTools.notNull(dims, this);
		String dec = new String(Base64.decode(data.getText(), Base64.GZIP));
		StreamTokenizer st = new StreamTokenizer(new StringReader(dec));
	...
	public String serialize(IFileFragment iff) throws IOException {
		String filename = iff.getAbsolutePath().substring(0,
