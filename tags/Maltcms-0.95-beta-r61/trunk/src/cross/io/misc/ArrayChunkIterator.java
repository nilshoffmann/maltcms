 ...
 * 
 */
public class ArrayChunkIterator implements Iterator<Array> {
	...
	public ArrayChunkIterator(final IVariableFragment ivf1, final int chunksize1) {
		...
		if (this.ivf.getArray() == null) {
			try {
				DataSourceFactory.getInstance().getDataSourceFor(
	...
	private Array loadChunk(final int i) {
		...
		if (r != null) {
			...
			try {
				...
				a = DataSourceFactory.getInstance().getDataSourceFor(
				        this.ivf.getParent()).readSingle(this.ivf);
