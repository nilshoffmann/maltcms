...
public class FragmentStringParser {
	...
	public IFileFragment parse(final String s) {
		...
		if (!s.contains(">")) {
			final IFileFragment ff = FileFragmentFactory.getInstance()
			        .createFragment(s, null);
			return ff;
		}
		...
		final Range irnge = null, vrnge = null;

		final IFileFragment parent = FileFragmentFactory.getInstance().create(
		        new File(dir, filename), this.getClass());
		if (vars.length == 0) {
			this.log.info("Vars not given explicitly, loading all!");
			try {
				DataSourceFactory.getInstance().getDataSourceFor(parent)
