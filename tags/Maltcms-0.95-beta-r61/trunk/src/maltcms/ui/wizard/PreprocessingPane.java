...
public class PreprocessingPane extends WizardPage {
    ...
    private void initComponents() {
        ...
        jList2.setName("maltcms.commands.fragments.DenseArrayProducer.maskMasses"); // NOI18N
        ...
        jCheckBox1.setName("maltcms.commands.fragments.DenseArrayProducer.invertMaskedMasses");
        ...
        if(evt.getActionCommand().equals("addMassBin")) {
            if(this.jTextField7!=null) {
                ...
                if(!s.isEmpty()){
                    ...
                    putWizardData("maltcms.commands.fragments.DenseArrayProducer.maskMasses",massVals.toString().substring(0, massVals.length()-1));
                }
            }
        }
    ...
    private void jButton5removeMassBinFilter(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton5removeMassBinFilter
        if(evt.getActionCommand().equals("removeMassBin")) {
            if(this.jTextField7!=null) {
                String s = this.jTextField7.getText();
                //have text in text field and no entry selected -> remove all entries in text field from model
                if(!s.isEmpty() && this.jList2.getSelectedIndices().length==0){
                    String[] masses = s.split(" ");
                    for(String mass:masses) {
                        Integer m = Integer.parseInt(mass);
                    ...
                    putWizardData("maltcms.commands.fragments.DenseArrayProducer.maskMasses","");
                }else{
                    ...
                    putWizardData("maltcms.commands.fragments.DenseArrayProducer.maskMasses",sb.toString().trim().replaceAll(" ", ","));
                }
            }
        }
