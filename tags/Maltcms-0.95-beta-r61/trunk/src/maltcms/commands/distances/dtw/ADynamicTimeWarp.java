...
public abstract class ADynamicTimeWarp implements IDynamicTimeWarp {
	...
	private IFileFragment calcAlignment(
	        final Tuple2D<IFileFragment, IFileFragment> t) {
		this.pa = Factory.getInstance().instantiate(PairwiseAlignment.class);
		pa.setIWorkflow(getIWorkflow());
		pa.setFileFragments(t.getFirst(), t.getSecond(), this.getClass());
		...
		pa.setTraceMatrix(this.predecessors);
		pa.setAlignment(this.alignment);
		pa.setPairwiseDistances(this.distance);
		...
		final StatsMap sm = ass.apply(new Array[] { this.distance.flatten()
		        .getSecond() })[0];
		pa.setIsMinimizing(minimize());
		pa.setCumulativeDistance(this.cd);
		pa.setPairwiseDistance(getPairwiseScanDistance());
		pa.setNumberOfScansReference(this.ref_num_scans);
		pa.setNumberOfScansQuery(this.query_num_scans);
		if (getStatsMap() != null) {
		...
		this.resF = pa.provideFileFragment();
		this.res = pa.getResult();
		this.resV = pa.getResultVector();
		...
		final BufferedImage bi = ArrayFactory.getInstance().createLayoutImage(
		        this.distance);
		Color c = Color.white;
		List<Tuple2DI> l = pa.getPath();
		...
		for (Tuple2D<Integer, Integer> pair : aps.getCorrespondingScans()) {
			g2.fillRect(pair.getSecond(), pair.getFirst(), 1, 1);
		}
		try {
			ImageIO.write(bi, "PNG", new File(FileTools.prependDefaultDirs(this
			        .getClass(), getIWorkflow().getStartupDate()), StringTools
	...
	protected void calculateCumulativeDistances(final IArrayD2Double distance2,
		...
		// cd.set(compression_penalty, expansion_penalty, diagonal_penalty);

		this.log.debug("Set up anchors!");
		double percentDone = 0;
		final int parts = 10;
		int partCnt = 0;
		final int elements = this.alignment.getNumberOfStoredElements();
	...
	public void configure(final Configuration cfg) {
		...
		setPairwiseScanDistance(Factory.getInstance().instantiate(
		...
		final CumulativeDistance cdist = Factory.getInstance().instantiate(
