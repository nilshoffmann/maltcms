...
public class ChromatogramVisualizer extends AFragmentCommand {
	...
	private Array filterBaseline(final Array tic, int median_window) {
		double current = 0;
	...
	public TupleND<IFileFragment> apply(final TupleND<IFileFragment> t) {
		...
		for (final IFileFragment ff : t) {
			...
			List<Array> eics = ArrayTools.tilt(aa);
			List<Array> feics = new ArrayList<Array>();
			// for (Array a : eics) {
			...
			log.info("SDev of intensities: {}, relative: {}", sdev, sdevrel);
			...
			if (!aa.isEmpty()) {
				...
				ImageTools.saveImage(bi, ff.getName() + "-chromatogram",
				        this.format, this.getClass(), this);
