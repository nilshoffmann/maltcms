...
public class PairwiseAlignmentVisualizer extends AFragmentCommand {
	...
	private void calcRMSE(final List<Tuple2DI> map1, final IFileFragment ref,
		...
		final double RMSE = ArrayTools.rootMeanSquareError(map1, ArrayTools
		        .normalizeGlobal(MaltcmsTools.getBinnedMZIs(ref).getSecond()),
		        ArrayTools.normalizeGlobal(MaltcmsTools.getBinnedMZIs(query)
		                .getSecond()));
	...
	public void configure(final Configuration cfg) {
		this.total_intensity = cfg.getString("var.total_intensity",
		        "total_intensity");
		this.y_axis_label  = cfg.getString(this.getClass().getName()+".y_axis_label","TIC");
		...
		this.createComparativeTICChart = cfg.getBoolean(this.getClass()
		        .getName()
		        + ".createComparativeTICChart", true);
		this.createDifferentialTICChart = cfg.getBoolean(this.getClass()
		        .getName()
		        + ".createDifferentialTICChart", true);
		this.createRatioTICChart = cfg.getBoolean(this.getClass().getName()
		        + ".createRatioTICChart", true);
		this.createSuperimposedTICChart = cfg.getBoolean(this.getClass()
		        .getName()
	...
	public Collection<XYAnnotation> getAnnotations(final IFileFragment iff,
		...
		for (final String s : cnames) {
			...
			final XYPointerAnnotation xyp = new XYPointerAnnotation(
			        s
			                + " "
			                + String.format("%1.2f", domain.getDouble(domIndex
			                        .set(v))), domain
			                .getDouble(domIndex.set(v)), yVals.getDouble(yValsI
			                .set(v))
	...
	public void makeComparativeTICChart(final AChart<XYPlot> tc1, final AChart<XYPlot> tc2,
	        final IFileFragment ref, final IFileFragment query,
	        final String x_label, final String y_label,
	        final Collection<XYAnnotation> c1,
	...
	public void makeSuperimposedChart(final List<Tuple2DI> map1,
		...
		this.log.info(ref.toString());
		this.log.info(query.toString());
		this.log.debug("Shapes of arrays:ref {}, mapped {}", Arrays
	...
	public void setFragments(final IFileFragment filea1,
		...
		try {
			...
			this.log.debug("Using scan acquisition time1 {}", domains[1]);
		...
		if (this.createComparativeTICChart) {
			makeComparativeTICChart(tc1, tc2,
			        filea1, fileb1, x_label,  this.y_axis_label, c1, c2,
			        min, max);
		}
		if (this.createDifferentialTICChart) {
			makeDifferentialTICChart(MaltcmsTools.getWarpPath(alignment),
			        filea1, fileb1, a1, b1, domains[0], x_label,  this.y_axis_label, c1, c2);
		}
		if (this.createRatioTICChart) {
			makeRatioTICChart(MaltcmsTools.getWarpPath(alignment), filea1,
			        fileb1, a1, b1, domains[0], x_label,  this.y_axis_label, c1, c2);
		}
