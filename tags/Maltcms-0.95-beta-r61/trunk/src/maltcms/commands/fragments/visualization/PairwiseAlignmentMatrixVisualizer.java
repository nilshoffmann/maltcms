...
import cross.tools.TiledImageTools;
...
public class PairwiseAlignmentMatrixVisualizer extends AFragmentCommand {
	...
	protected BufferedImage addSpectra(final String queryName,
		...
		this.log.info("Adding Spectra!");
		this.log.info("Reference {} with length: {}", refName, ref.getSize());
		this.log.info("Query {} with length: {}", queryName, query.getSize());
		this.log.info("TiledImage size: (wxh) " + bi.getWidth() + " x "
		        + bi.getHeight());
		...
		this.log.info("Reference img length: " + bi_ref.getWidth());
		this.log.info("Query img length: " + bi_query.getWidth());
		final BufferedImage bout = TiledImageTools.prepareImage(
		        bi.getWidth() + specwidth1 + colorlegendwidth + (2 * margin)
		                + 1, bi.getHeight() + (2 * margin) + specwidth1 + 1,
		        1000, 1000, bi.getColorModel()).getAsBufferedImage();
		...
		this.log.info("Adding {} on top!", queryName);
		g.drawImage(bi_query, at, null);

		g.setTransform(AffineTransform.getTranslateInstance(0.0d, 0.0d));
		this.log.info("Filling distance matrix!");
		at = AffineTransform.getTranslateInstance(specwidth1
		...
		this.log.info("Setting base for color table to x={},y={}", bout
		        .getWidth()
	...
	public TupleND<IFileFragment> apply(final TupleND<IFileFragment> t1) {
		final IFileFragment iff = MaltcmsTools.getPairwiseDistanceFragment();
		TupleND<IFileFragment> pwdt = null;
		if (iff == null) {// No alignment has been calculated in this run
			this.log.info("Running stand-alone");
	...
	public void configure(final Configuration cfg) {
		...
		this.colorramp_location = cfg.getString("images.colorramp",
		        "jar:file:maltcms-res.jar!/res/colorRamps/bw.csv");
	...
	protected void createAlignmentMatrixImage(final Array a,
		...
		this.log.info("Height of IChromatogram: " + this.chromatogramHeight);
		final int rows = a.getShape()[0];
		final int cols = a.getShape()[1];
		this.log.info("Size of Image {}x{} = {}", new Object[] { cols, rows,
		        (cols * rows) });
		BufferedImage ti = new BufferedImage(cols, rows,
		        BufferedImage.TYPE_INT_RGB);
		...
		final WritableRaster wr = ti.getRaster();
		...
		this.log.debug("{}", Arrays.toString(breakpoints));
		this.log.info("Drawing image with {} rows and {} cols", rows, cols);
		double percentDone = 0.0d;
		final int elements = cols * rows;
		int elemCnt = 0;
		final int parts = 10;
		int partCnt = 0;
		...
		for (int i = 0; i < rows; i++) {
			for (int j = 0; j < cols; j++) {
				...
				if ((av == Double.POSITIVE_INFINITY)
					...
					this.log.debug("Found infinite value at {},{}", i, j);
					infinite = true;
				} else {
		...
		if ((reference != null) && (query != null)) {
			...
			final List<Tuple2D<Integer, Integer>> l = aps
			...
			ti = addSpectra(qF.getName(), rF.getName(), query, reference, ti,
			        this.chromatogramHeight, 50, 255, 100, colorRamp, minimize,
			        l1, l2);
		}

		this.log.info("Writing Image to file {}", output);
		TiledImageTools.saveImage(ti, output, this.format);
	...
	private void createImage(final IFileFragment f) {
		for (final String s : this.matrix_vars) {
			try {
				...
				IArrayDoubleComp iadc = Factory.getInstance().instantiate(ac,
				        IArrayDoubleComp.class);
				boolean minimize = iadc.minimize();
				if ((mm.min == Double.NEGATIVE_INFINITY)
				        && (mm.max == Double.POSITIVE_INFINITY)) {
					this.log
					        .warn("Found -INF as minimum and +INF as maximum, can not create image!");
					return;
				}
				...
				final IVariableFragment query_spec = queryF
				        .getChild(this.top_chromatogram_var);
				final IVariableFragment reference_spec = refF
				        .getChild(this.left_chromatogram_var);
				...
				if ((qval.getRank() == 1) && (rval.getRank() == 1)) {// ensure
					// equal
					// ranks
					final IVariableFragment path_i_fragment = f
					        .getChild(this.path_i);
					final IVariableFragment path_j_fragment = f
					        .getChild(this.path_j);
					...
					createAlignmentMatrixImage(matrix, matrix_frag,
					        ...
					        qval, rval, l, FileTools.prependDefaultDirs(
					                this.filename, this.getClass(),
					                getIWorkflow().getStartupDate()), minimize);
					System.gc();
				} else {
				...
				this.log.warn("Could not load variable {}", s);
			}
		}
	}
