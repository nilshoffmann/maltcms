...
public class ChromatogramWarp extends AFragmentCommand {
	...
	public IFileFragment copyReference(final IFileFragment ref,
	        final IWorkflow iw) {
		try {
			DataSourceFactory.getInstance().getDataSourceFor(ref)
			        .readStructure(ref);
		} catch (final IOException e) {
			throw new RuntimeException(e.fillInStackTrace());
		}
		final IFileFragment copy = FileFragmentFactory.getInstance().create(
		        FileTools.prependDefaultDirs(StringTools.removeFileExt(ref
		                .getName())
		                + ".cdf", this.getClass(), iw.getStartupDate()));
	...
	public IFileFragment warp(final IFileFragment ref,
		...
		this.log.info("Warping {}, saving in {}", originalFile
		        .getAbsolutePath(), processedFile.getAbsolutePath());
		final IFileFragment warped = FileFragmentFactory.getInstance().create(
		        FileTools.prependDefaultDirs(StringTools
		                .removeFileExt(originalFile.getName())
		                + ".cdf", this.getClass(), iw.getStartupDate()));
		try {
			DataSourceFactory.getInstance().getDataSourceFor(originalFile)
			        .readStructure(originalFile);
		} catch (final IOException e) {
			throw new RuntimeException(e.fillInStackTrace());
		}
		warped.setAttributes(originalFile.getAttributes().toArray(
		        new Attribute[] {}));
		this.log.debug("Currently set attributes on file {}:{}", originalFile
		        .getAbsolutePath(), Arrays.deepToString(originalFile
		        .getAttributes().toArray()));
		this.log.debug("Currently set attributes on file {}:{}", warped
		        .getAbsolutePath(), Arrays.deepToString(warped.getAttributes()
		        .toArray()));
		this.log.debug("Warping {} to {}, saving in {}", new Object[] {
	...
	public IFileFragment warp1D(final IFileFragment warpedB,
		...
		for (final String s : plainVars) {
			...
			this.log.debug("Warping variable {}", s);
	...
	public IFileFragment warp2D(final IFileFragment warpedB,
		...
		// }
		try {
			this.log.debug("Processing {} indexed by {} from file {}",
	...
	public void warpAnchors(final IFileFragment warped,
		...
		final ArrayChar.D2 anchNames = ArrayTools.createStringArray(l.size(),
		        1024);
		for (int j = 0; j < l.size(); j++) {
			if (toLHS) {
				int warpedAnchor = ArrayTools.getNewIndexOnLHS(l.get(j)
				        .getScanIndex(), path);
				anchPos.set(j, warpedAnchor);
				anchNames.setString(j, l.get(j).getName());
			} else {
				int warpedAnchor = ArrayTools.getNewIndexOnRHS(l.get(j)
				        .getScanIndex(), path);
				anchPos.set(j, warpedAnchor);
				anchNames.setString(j, l.get(j).getName());
			}
		...
		try {
			anchorScanIndex = warped.getChild(this.anchorScanIndexVariableName);
		} catch (final ResourceNotAvailableException rnae) {
			anchorScanIndex = new VariableFragment(warped,
			        this.anchorScanIndexVariableName);
			this.log.warn("{}", rnae.getLocalizedMessage());
		}
		anchorScanIndex.setArray(anchPos);
		IVariableFragment anchorName = null;
		try {
			anchorName = warped.getChild(this.anchorNameVariableName);
		} catch (final ResourceNotAvailableException rnae) {
			anchorScanIndex = new VariableFragment(warped,
			        this.anchorScanIndexVariableName);
			this.log.warn("{}", rnae.getLocalizedMessage());
