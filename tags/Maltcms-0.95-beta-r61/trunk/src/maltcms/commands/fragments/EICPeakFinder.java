...
public class EICPeakFinder extends AFragmentCommand {
	...
	protected Tuple2D<Integer, Double> getMaxAutocorrelation(final Array a,
		...
		Index ia = a.getIndex();
		Index ib = b.getIndex();
		ArrayDouble.D1 autoCorr = new ArrayDouble.D1(a.getShape()[0]);
	...
	public TupleND<IFileFragment> apply(final TupleND<IFileFragment> t) {
		...
		for (final IFileFragment f : t) {
			...
			final StatsMap[] sm = ass
			        .apply(al.toArray(new ArrayDouble.D1[] {}));
			final CSVWriter csvw = new CSVWriter();
			csvw.setIWorkflow(getIWorkflow());
			csvw.writeOneFilePerArray(FileTools.prependDefaultDirs(
			...
			final ArrayDouble.D2 corrs = new ArrayDouble.D2(al.size(), al
			        .size());
			...
			final IFileFragment iff = FileFragmentFactory.getInstance().create(
			        FileTools.getDefaultDirs(getIWorkflow().getStartupDate())
			                .getAbsolutePath(),
			        StringTools.removeFileExt(f.getName())
			                + "-eic_correlation.cdf", null, this.getClass());
			final IVariableFragment ivf = FragmentTools.createVariable(iff,
			        "eic-correlation", null);
			ivf.setArray(corrs);
			ArrayList<Integer> heights = new ArrayList<Integer>();
			ArrayList<String> eics = new ArrayList<String>();
			for (int i = 0; i < al.size(); i++) {
				heights.add(Integer.valueOf(20));
				eics.add("EIC " + i);
			}
			RenderedImage bi = ImageTools.makeImage2D(corrs, 1024,
			        Double.POSITIVE_INFINITY);
			ImageTools.saveImage(bi, "eic_correlation_"
			        + StringTools.removeFileExt(iff.getName()), "png", this
			        .getClass(), this);
	...
	private void calcCrossCorr(
		...
		for (int i = 0; i < n; i++) {
			final Tuple2D<Tuple2D<Integer, Integer>, Double> r = ecs.take()
			        .get();
			if (r != null) {
