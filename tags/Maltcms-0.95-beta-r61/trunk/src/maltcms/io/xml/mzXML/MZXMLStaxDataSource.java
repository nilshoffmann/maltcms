...
public class MZXMLStaxDataSource implements IDataSource {
	...
	private final String[] fileEnding = new String[] { "mzxml", "xml" };
	...
	public int canRead(final IFileFragment ff) {
		...
		final String fileending = StringTools.getFileExtension(ff.getName());
		getParser(ff);
		if (dotindex == -1) {
	...
	private MSXMLParser getParser(IFileFragment ff) {
		return getParser(ff.getAbsolutePath());
	}

	private MSXMLParser getParser(String file) {
	...
	protected Tuple2D<Array, Array> initMinMaxMZ(final IVariableFragment var,
		...
		ArrayTools.fill(mass_range_min1, Math.rint(min_mass));
		ArrayTools.fill(mass_range_max1, Math.rint(max_mass));
	...
	private int getScans(final MSXMLParser mp, final int mslevel) {
		...
		SortedMap<Integer, Integer> mslevels = new TreeMap<Integer, Integer>();
		for (int i = 0; i < mp.getScanCount(); i++) {
			ScanHeader h = mp.rapHeader(i + 1);
			if (mslevels.containsKey(Integer.valueOf(h.getMsLevel()))) {
				Integer val = mslevels.get(Integer.valueOf(h.getMsLevel()));
				mslevels.put(h.getMsLevel(), Integer.valueOf(val + 1));
			} else {
				mslevels.put(h.getMsLevel(), Integer.valueOf(1));
			}
		...
		this.log.info("{}", mslevels.toString());
		return scans;
	}
	...
	private ArrayList<Array> readMZIScans(final IVariableFragment var,
		...
		if (var.getVarname().equals(this.mass_values)) {
			...
			for (int i = start; i < scans; i++) {
				...
				final Scan s = mp.rap((i + 1));
				if (s.getHeader().getMsLevel() == mslevel) {
					final ArrayDouble.D1 a = new ArrayDouble.D1(s.header
					        .getPeaksCount());
					this.log.debug("Converting scan {} of {} with {} peaks",
					        new Object[] { (i + 1), scans,
					                s.header.getPeaksCount() });
	...
	private Array readScanAcquisitionTimeArray(final IVariableFragment var,
		...
		for (int i = start; i < scans; i++) {
			final Scan s = mp.rap((i + 1));
			sat.set(i, s.header.getDoubleRetentionTime());
		}
		// f.setArray(sat);
		return sat;
	}
	...
	private Array readScanIndex(final IVariableFragment var,
		...
		int skipped = 0;
		for (int i = start; i < mp.getScanCount(); i++) {
			final Scan s = mp.rap((i + 1));
			if (s.getHeader().getMsLevel() == mslevel) {
				...
				scan_index.set(i - skipped, npeaks);
				npeaks += s.header.getPeaksCount();
			} else {
				skipped++;
			}
	...
	public Array readSingle(final IVariableFragment f) throws IOException,
	        ResourceNotAvailableException {
		this.log.info("readSingle of {} in {}", f.getVarname(), f.getParent()
		        .getAbsolutePath());
		if (f.hasArray()) {
	...
	private Array readTotalIntensitiesArray(final IVariableFragment var,
		...
		final ArrayDouble.D1 total_intensities = new ArrayDouble.D1(scans);
		for (int i = start; i < scans; i++) {
			final ScanHeader s = mp.rapHeader((i + 1));
			if (s.getMsLevel() == mslevel) {
				total_intensities.set(i, s.getTotIonCurrent());
			}
		}
	...
	protected Scan toScan(final int num, final Array mz, final Array i) {
		...
		s.header.setTotIonCurrent((float) ArrayTools.integrate(i));
		return s;
	}
