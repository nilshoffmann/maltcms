...
public class ArrayFactory {
	...
	private static ArrayFactory f = null;

	/**
	 * Retrieve a singleton instance of ArrayFactory.
	 * 
	 * @return
	 */
	public static ArrayFactory getInstance() {
		if (ArrayFactory.f == null) {
			ArrayFactory.f = new ArrayFactory();
		}
		return ArrayFactory.f;
	}
	...
	public IArrayD2Double create(final int rows, final int cols,
		...
		if (aps.getSize() > 2) {
			...
			return PartitionedArray.create(rows, cols, default_value,
			        PartitionedArray.createBandConstraint(0, 0, rows, cols,
			                band));
		}
