...
public class PartitionedArray implements IArrayD2Double {
	...
	public static Area calculateLayout(final int rows, final int cols,
		...
		EvalTools.eqI(x, 0, PartitionedArray.class);// check our assertion
		EvalTools.eqI(y, 0, PartitionedArray.class);// check our assertion

		int cnt = 0;// no anchors processed so far
		// int row = 0;//row counter
		int roverlap = rowoverlap;
		int coverlap = coloverlap;
		...
		for (final Tuple2D<Integer, Integer> t : ris) {// iterate over anchors
			...
			if (band > 0.0d && band < 1.0d) {
				Area bandArea = createBandConstraint(j0, i0, r.height, r.width,
				        band);
				Area b = new Area(r);
		...
		for (final Rectangle r : distinctRects) {
			Logging.getLogger(PartitionedArray.class).debug(r.toString());
		}
		return bounds;
	}
	...
	public static BufferedImage createLayoutImage(PartitionedArray pa,
	        Color bg, Color fg) {
		BufferedImage bi = new BufferedImage(pa.columns(), pa.rows(),
	...
	public static PartitionedArray create(final int rows, final int cols,
		...
		Area shape = null;
		if (globalBand) {
			shape = PartitionedArray.calculateLayout(rows, cols, neighborhood1,
			        aps, -1, 0, 0);
			if ((band > 0.0) && (band < 1.0)) {
				final Area bandshape = PartitionedArray.createBandConstraint(0,
				        0, rows, cols, band);
	...
	public static Area createBandConstraint(final int x, final int y,
	        final int rows, final int cols, final double r) {
		final double maxdev = Math.ceil(Math.max(rows, cols) * r);
		final double ascent = (double) cols / (double) rows;
		Logging
		        .getLogger(PartitionedArray.class)
		        .info(
	...
	public static void main(final String[] args) {
		...
		final AnchorPairSet aps = new AnchorPairSet(al1, al2, 15, 16);
	...
	public Tuple2D<D1, ucar.ma2.ArrayDouble.D1> flatten() {
		ArrayDouble.D1 arr = new ArrayDouble.D1(getNumberOfStoredElements());
		ArrayInt.D1 si = new ArrayInt.D1(rows());
		int offset = 0;
		for (int i = 0; i < rows(); i++) {
			si.set(i, 0 + offset);
			int[] bds = getColumnBounds(i);
			int ecnt = 0;
