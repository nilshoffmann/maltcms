...
public class MZIWarpInput implements IWarpInput {
	...
	public MZIWarpInput(final IFileFragment ff, final IWorkflow iw) {
		...
		final List<Array> i1 = MaltcmsTools.getBinnedMZIs(referenceFile).getSecond();
		final List<Array> i2 = MaltcmsTools.getBinnedMZIs(queryFile1).getSecond();
		final Tuple2D<List<Array>, List<Array>> t = new Tuple2D<List<Array>, List<Array>>(
		        i1, i2);
		final IFileFragment target = FragmentTools
		        .createFragment(referenceFile, queryFile1, this.getClass(), iw
		                .getStartupDate());
