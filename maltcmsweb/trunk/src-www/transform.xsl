...
<xsl:transform version="2.0"
						...
						<xsl:element name="html">
							...
							<xsl:element name="head">
								...
								<xsl:element name="link">
									...
									<xsl:attribute name="href"><xsl:value-of
										select="./@toRoot" /><xsl:text>main.css</xsl:text></xsl:attribute>
							...
							<xsl:element name="body">
								...
								<xsl:apply-templates select="child::node()">
									<xsl:with-param name="PAGENAME" select="$pagename" />
									<xsl:with-param name="GROUPNAME" select="$pagename" />
								</xsl:apply-templates>
								<!-- </div> -->
								<div id="footer">
									<xsl:call-template name="COPYRIGHT" />
									2008-2010,
									<a target="_self" title="Contact" href="../info/contact.html">Nils Hoffmann</a> | Maltcms is hosted by <a target="_blank" title="Hosting by sourceforge.net" href="http://sourceforge.net/">sourceforge.net</a>
								</div>
							</xsl:element>
						</xsl:element>
					</xalan:write>
	...
	<xsl:template match="ref" name="REFTEMPLATE">
		...
		<xsl:element name="a">
			<xsl:attribute name="href"><xsl:value-of select="$PATHCORRECTION" />
					<xsl:value-of select="./@target" /><xsl:text>.html</xsl:text>
				</xsl:attribute>
	...
	<xsl:template match="column" name="COLUMNTEMPLATE">
		<xsl:element name="div">
			<xsl:call-template name="ATTRCLASSTEMPLATE" />
			<xsl:element name="div">
				<xsl:attribute name="id"><xsl:text>pagename</xsl:text></xsl:attribute>
				<xsl:value-of select=".././@nname" />
			</xsl:element>
			...
			<xsl:element name="div">
				<xsl:attribute name="id"><xsl:text>colcontent</xsl:text></xsl:attribute>
				<xsl:apply-templates />
			</xsl:element>
		</xsl:element>
	</xsl:template>
	...
	<xsl:template match="heading" name="HEADINGTEMPLATE">
		<xsl:element name="div">
			<xsl:element name="span">
				<xsl:attribute name="class">
					<xsl:text>heading</xsl:text><xsl:value-of select="./@level" />
				</xsl:attribute>
			...
			<xsl:element name="ul">
				<xsl:if test="$listclass!=''">
					<xsl:attribute name="class"><xsl:value-of
						select="$listclass" /></xsl:attribute>
				</xsl:if>
				<xsl:apply-templates />
			</xsl:element>
		</div>
