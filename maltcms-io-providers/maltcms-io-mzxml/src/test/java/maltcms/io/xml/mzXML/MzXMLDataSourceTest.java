...
import org.systemsbiology.jrap.staxnxt.TestParser;
...
public class MzXMLDataSourceTest {
    
    private static final org.slf4j.Logger log = LoggerFactory.getLogger(MzXMLDataSourceTest.class);
    ...
    @Rule
    public ExtractClassPathFiles ecpf = new ExtractClassPathFiles(tf, "/mzXML/R1_RG59_B4_1.mzXML.gz");//, "/mzXML/tiny1.mzXML2.0.mzXML.gz", "/mzXML/tiny1.mzXML3.0.mzXML.gz");

    MZXMLDataSource getDataSource() {
        return new MZXMLDataSource();
    }
    ...
    @Before
    public void setUp() {
        Factory.getInstance().getDataSourceFactory().setDataSources(Arrays.asList(MZXMLDataSource.class.getCanonicalName()));
    ...
    public void testReadAll() throws Exception {
        ...
        sl.setLogLevel("maltcms.io.xml.mzXML", "DEBUG");
        for (File f : ecpf.getFiles()) {
    ...
    public void testSupportedFormats() {
        String[] fileEnding = new String[]{"mzxml", "mzxml.xml"};
        List<String> supportedFormats = new MZXMLDataSource().supportedFormats();
        Arrays.sort(fileEnding);
        Collections.sort(supportedFormats);
        Assert.assertEquals(supportedFormats, Arrays.
                asList(fileEnding));
    }
...
//        FileFragment.clearFragments();
        ...
        try {
            ...
            Logger.getLogger(MzXMLDataSourceTest.class.getName()).
