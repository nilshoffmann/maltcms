...
//            Assert.assertTrue(tic.getRange()[0].toString().equals(si.getRange()[0].toString()));
    ...
    public void testSupportedFormats() {
        String[] fileEnding = new String[]{"mzml", "mzml.xml"};
        Assert.assertEquals(new MZMLDataSource().supportedFormats(), Arrays.
            asList(fileEnding));
    }
...
//        FileFragment.clearFragments();
        ...
        try {
            ...
            Logger.getLogger(MZMLDataSourceTest.class.getName()).
...
//        FileFragment.clearFragments();
    ...
    public void testSavedVariableFragmentMissingSourceFile() {
        IFileFragment f = createTestFragment();
        f.addSourceFile(new FileFragment(new File("/this/file/does/not/exist.mzML")));
        f.save();
        //first check that all are there
        Assert.assertNotNull(f.getChild("variable1"));
        Assert.assertNotNull(f.getChild("variable2"));
