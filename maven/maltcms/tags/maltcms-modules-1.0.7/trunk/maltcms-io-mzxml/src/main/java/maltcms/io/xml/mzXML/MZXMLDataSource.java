...
public class MZXMLDataSource implements IDataSource {
	...
	public void configure(final Configuration configuration) {
		final List<String> dss = StringTools.toStringList(configuration
		        .getList(getClass().getName() + ".dataSources", Arrays.asList(
		...
		for (final IDataSource ids : this.ds) {
			try {
				this.log.info("Checking DataSource {}", ids.getClass()
				        .getName());
				if (ids.canRead(ff) > 0) {
					this.fragmentToValidReaderMap
					        .put(ff.getAbsolutePath(), ids);
					return ids;
				}
