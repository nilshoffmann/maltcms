 ...
 * 
 * $Id: Array2DTimePenalized.java 161 2010-09-07 19:14:30Z mwilhelm42 $
 */
...
public class Array2DTimePenalized implements IArrayDoubleComp {
	...
	public Double apply(final int i1, final int i2, final double time1,
		...
		if (time1 != 0.0d) {
			frtPenalty = Math
			        .exp(-((time1) * (time1) / (2.0d * this.frtTolerance * this.frtTolerance)));
		}
		double srtPenalty = 1.0d;
		if (time2 != 0.0d) {
			srtPenalty = Math
			        .exp(-((time2) * (time2) / (2.0d * this.srtTolerance * this.srtTolerance)));
		}
		double s = dotP * frtPenalty * srtPenalty;
		// System.out.println(time1 + ", " + time2 + ": " + s);
		return s;
	}
