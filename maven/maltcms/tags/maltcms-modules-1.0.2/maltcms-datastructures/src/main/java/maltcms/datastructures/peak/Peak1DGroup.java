 ...
 * 
 */
public class Peak1DGroup extends Peak1D implements Iterable<Peak1D> {
	...
	private TreeSet<Peak1D> l;

	public Peak1DGroup(Peak1D... p) {
		this.l = new TreeSet<Peak1D>(new Peak1DComparator());
	...
	public Array getMasses() {
		...
		Iterator<Peak1D> miter = this.l.iterator();
		int i = 0;
		while (miter.hasNext()) {
			Peak1D p = miter.next();
	...
	public String toString() {
		...
		Iterator<Peak1D> p = this.l.iterator();
		while (p.hasNext()) {
			sb.append(p.next() + "\n");
		}
		return sb.toString();
