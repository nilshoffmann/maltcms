 ...
 * 
 * $Id: CenterStarAlignment.java 160 2010-08-31 19:55:58Z nilshoffmann $
 */
...
public class CenterStarAlignment extends AFragmentCommand {
	...
	public TupleND<IFileFragment> apply(final TupleND<IFileFragment> t) {
		...
		final IFileFragment centerSeq = Factory.getInstance()
		        .getFileFragmentFactory().create(names1.getString(center));
	...
	private void addMultipleAlignment(final IFileFragment f,
		...
		ArrayChar.D2 names = cross.datastructures.tools.ArrayTools.createStringArray(table
		        .size(), 1024);
		for (int j = 0; j < table.size(); j++) {
	...
	private List<IFileFragment> saveAlignment(final Class<?> creator,
		...
		IFileFragment maFragment = new FileFragment(getIWorkflow()
	...
	private void saveScanAcquisitionTimes(final TupleND<IFileFragment> t,
		...
		csv.writeTableByCols(getIWorkflow().getOutputDirectory(this)
	...
	private File saveMultipleAlignmentTable(final Class<?> creator,
		...
		final CSVWriter csv = Factory.getInstance().getObjectFactory()
		        .instantiate(CSVWriter.class);
		csv.setIWorkflow(getIWorkflow());
		return csv.writeTableByCols(getIWorkflow().getOutputDirectory(this)
	...
	private void prepareFileFragments(final TupleND<IFileFragment> alignment,
		...
		FileFragment newTop = new FileFragment(getIWorkflow()
		        .getOutputDirectory(this), top.getName());
		...
		for (final IFileFragment iff : alignment) {
			...
			if (lhsName.equals(refname)) {
				...
				FileFragment nrhs = new FileFragment(getIWorkflow()
				...
				FileFragment nlhs = new FileFragment(getIWorkflow()
