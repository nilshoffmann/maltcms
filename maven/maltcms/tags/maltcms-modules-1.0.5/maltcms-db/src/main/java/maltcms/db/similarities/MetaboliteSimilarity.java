...
public class MetaboliteSimilarity extends Similarity<IMetabolite> {
	...
	public double get(Tuple2D<Array, Array> query, IMetabolite t1) {
		...
		Tuple2D<Array, Array> tpl1 = new Tuple2D<Array, Array>(t1
		        .getMassSpectrum().getFirst(), t1.getMassSpectrum().getSecond());
	...
	protected double similarity(Tuple2D<Array, Array> t1,
		...
		int bins = MaltcmsTools
		        .getNumberOfIntegerMassBins(min, max, resolution);
		...
		// if(delta>maxdelta) {//check, which one is larger and advance smaller
		// one
		// score+=(scorel*scorer);
		// scorel = 0;
		// scorer = 0;
		...
		// }else{
		// throw new
		// IllegalStateException("Cannot advance both sides if delta is too large!");
		// //advanceWhich = 0;
		// }
		// }else{//delta<=maxdelta
		...
		if (this.lastT != null) {
			...
			ArrayTools.createDenseArray(t1.getFirst(), t1.getSecond(),
			        new Tuple2D<Array, Array>(dmasses1, s1), ((int) Math
			                .floor(min)), ((int) Math.ceil(max)), bins,
			        resolution, 0.0d);
		...
		// //s1 = new
		// ArrayInt.D1(tpl1.getFirst(),tpl1.getSecond(),(int)min,(int)max);
		// // hm.put(t1,s1);
		// // }
		...
		int[] midx = MaltcmsTools.getTopKIndicesOfMaxIntensity(s2, 10);
		boolean amongTopK = false;
		for (int i = 0; i < midx.length; i++) {
			if (this.mw >= 0 && dmasses2.getInt(midx[i]) == this.mw) {
				System.out.println("Molecular weight match detected for mw: "
				        + this.mw);
				amongTopK = true;
			}
		}
		...
		//		
		//		
		// // Array indx1 = Array.factory(tpl1.getFirst().getElementType(), new
		// int[] { bins });
		// // Array vals1 = Array.factory(tpl1.getSecond().getElementType(), new
		// int[] { bins });
		// //
		// // ArrayTools.createDenseArray((Array)tpl1.getFirst(),
		// (Array)tpl1.getSecond(), new Tuple2D<Array,Array>(indx1, vals1),
		// // ((int) Math.rint(min)), ((int) Math.rint(max)),bins,0.0d);
		// //
		// // Array indx2 = Array.factory(tpl2.getFirst().getElementType(), new
		// int[] { bins });
		// // Array vals2 = Array.factory(tpl2.getSecond().getElementType(), new
		// int[] { bins });
		// // ArrayTools.createDenseArray((Array)tpl2.getFirst(),
		// (Array)tpl2.getSecond(), new Tuple2D<Array,Array>(indx2, vals2),
		...
		WeightedCosine wc = new WeightedCosine();
		DefaultFeatureVector dfv1 = new DefaultFeatureVector();
		dfv1.addFeature("binned_mass_values", dmasses1);
		dfv1.addFeature("binned_intensity_values", s1);
		DefaultFeatureVector dfv2 = new DefaultFeatureVector();
		dfv2.addFeature("binned_mass_values", dmasses2);
		dfv2.addFeature("binned_intensity_values", s2);
		double v = wc.apply(dfv1, dfv2);
