...
public class Clique {
	...
	private boolean handleConflictingPeak(Peak p) {
		...
		if (bbh1 > bbh2) {
			...
			log.warn("BBH count draw between peaks p:{}, q:{} with value: {}",
			        new Object[] { p, q, bbh1 });
	...
	public BoxAndWhiskerItem createApexTicBoxAndWhisker() {
		...
		for (IFileFragment f : this.clique.keySet()) {
			l.add(Math.log(ArrayTools.integrate(centroid.getMSIntensities()))
			        - ArrayTools.integrate(this.clique.get(f)
			                .getMSIntensities()));
		}
		return BoxAndWhiskerCalculator.calculateBoxAndWhiskerStatistics(l);
	}
	...
	public List<Peak> getPeakList() {
		...
		Collections.sort(peaks, new Comparator<Peak>() {
			...
			public int compare(Peak o1, Peak o2) {
				return o1.getAssociation().getName()
				        .compareTo(o2.getAssociation().getName());
			}
		});
		return peaks;
	}
