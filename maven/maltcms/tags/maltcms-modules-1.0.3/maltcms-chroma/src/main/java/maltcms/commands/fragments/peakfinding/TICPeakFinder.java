...
public class TICPeakFinder extends AFragmentCommand {
	...
	private List<Peak1D> findPeakAreas(final ArrayList<Integer> ts,
		...
		if (integrateTICPeaks) {
			...
			XYChart xyc = new XYChart(iff.getName() + "-TIC", new String[] {
			...
			final PlotRunner pr = new PlotRunner(xyc.create(),
			        "TIC and TIC derivatives " + iff.getName(),
			        "tic-derivatives-"
	...
	private IFileFragment findPeaks(final IFileFragment f) {
		...
		if (this.integratePeaks) {
			peaks = findPeakAreas(ts, f, this.epsilon, tic);
		...
		ff.addSourceFile(f);
		addResults(ff, correctedtic, extr, peaks);
		ff.save();
		f.clearArrays();
	...
	private Array applyFilters(final Array correctedtic) {
		...
		final Array filteredtic = BatchFilter
