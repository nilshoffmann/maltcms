 ...
 * 
 */
public class Scan2DBidiBestHitsCriterion implements
        ICliqueMemberCriterion<IScan2D> {

    public class Peak1DSimilarities {

        private final IArrayD2Double sims;
        private final List<IScan2D> peaks;

        public Peak1DSimilarities(List<IScan2D> peaks, IArrayD2Double sims) {
            this.peaks = peaks;
            this.sims = sims;
     ...
     */
    @Override
    public boolean shouldBeMemberOf(IClique<IScan2D> c, IScan2D t) {
        // test t against all members of c
        // if all are bidibesthits, t should be a member
        for (IScan2D p : c.getFeatureVectorList()) {
