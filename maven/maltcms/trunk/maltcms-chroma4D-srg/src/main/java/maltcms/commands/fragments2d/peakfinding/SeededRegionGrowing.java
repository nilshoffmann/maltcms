...
public class SeededRegionGrowing extends AFragmentCommand {
    ...
    private int count;
    private IArraySimilarity similarityFunction;
    ...
    public TupleND<IFileFragment> apply(final TupleND<IFileFragment> t) {
        ...
        for (final IFileFragment ff : t) {
            log.info("Setting up intensities({})", this.totalIntensityVar);
            ...
            log.debug("SPM: {}", this.scansPerModulation);
            ...
            final Double minIntensityThreshold = sm[0].get(Vars.Mean.toString())
                    + (sdev * this.peakFindingThreshold);// sm[0].get(Vars.Max.toString());
            List<Point> peaks = null;

            log.info("	Using {} as lower bound", minIntensityThreshold);
            ...
            log.info("	Found {} potential peaks in {}", peaks.size(), ff.
                    getName());

            log.info("Computing areas");
            log.info("	Using similarity {} with minDist:{}",
                    this.similarityFunction.toString(), this.minDistance);
            this.timesum = 0;
            this.count = 0;
        ...
        if ((t.size() > 1) && this.doBBH) {
            ...
            log.info("Exporting whole detailed BBH information");
            ...
            log.info("Exporting peak occurence map");
            ...
            for (int i = 0; i < this.bbh.getPeakLists().size(); i++) {
                for (int j = i + 1; j < this.bbh.getPeakLists().size(); j++) {
                    log.info(
                            ...
                            chromatogramNames.get(i), chromatogramNames.get(j));
    ...
    public void configure(final Configuration cfg) {
        ...
        distClass = cfg.getString(this.getClass().getName() + ".distClass",
                "maltcms.math.functions.similarities.ArrayCos");
        this.similarityFunction = Factory.getInstance().getObjectFactory().instantiate(
                this.distClass, IArraySimilarity.class);
        ...
        this.peakExporter = Factory.getInstance().getObjectFactory().instantiate(
                peakExporterClass, IPeakExporter.class);
        ...
        this.integration = Factory.getInstance().getObjectFactory().instantiate(
                peakIntegrationClass, IPeakIntegration.class);
        ...
        for (int x = xsize; x < scanLineCount - xsize; x++) {
            for (int y = ysize; y < this.scansPerModulation - ysize; y++) {
                ...
                if (max && (currentHeight > min)
                    ...
                    // log.info("Max mass at peak candidate: {}",MaltcmsTools.getMaxMass(t.getFirst(),t.getSecond()));
                }
            }
        }
        return peaks;
    ...
    private void getFilter(final IFileFragment ff) {
        if (this.filterMS) {
            ...
            try {
                final Array holdA = ff.getChild(this.usedMassValuesVar).getArray();
    ...
    private boolean isNear(final Array seedMS, final Array neighMS) {
        if ((seedMS != null) && (neighMS != null)) {
            ...
            final double d = this.similarityFunction.apply(sms, nms);
            // log.info("{}", d);
...
//            }
    ...
    private List<Peak2D> savePeaks(final IFileFragment ff,
        ...
        final Tuple2D<ArrayDouble.D1, ArrayDouble.D1> times = getRetentiontime(
                ff);
        ...
        log.info("Creating peaklist");
        final List<Peak2D> peaklist = createPeaklist(StringTools.removeFileExt(ff.
                getName()), peakAreaList, times, fret);
        ...
        TupleND<IFileFragment> iff = getWorkflow().getCommandSequence().getInput();
