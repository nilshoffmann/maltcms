...
public class PipelineWizard implements WizardResultProducer, HyperlinkListener,
    ...
    /**
     * Goals for v0.1 Configure a pipeline for <ul> <li>anchor
     * finding/loading</li> <li>binning, preprocessing</li> <li>alignment</li>
     * <li>visualization</li> </ul> and allow the user to decide on the first
     * pane, whether an existing pipeline configuration should be loaded, or if
    ...
    public static void main(String[] args) {
        System.setProperty("log4j.configuration",
                "file://${user.dir}/cfg/log4j.properties");
        ...
        Runnable r = new Runnable() {
            ...
            private void createChromAFrame() {
                ...
                final JSplitPane jsp = new javax.swing.JSplitPane(
                        JSplitPane.HORIZONTAL_SPLIT);
                final JEditorPane jep = new JEditorPane("text/html", "");
                StringBuilder sb = new StringBuilder();
                sb
                        .append("<h1 style=\"font-family:Arial,Tahoma,sans-serif;\">Welcome to ChromA</h1>");
                sb
                ...
                sb
                        .append("<p style=\"font-family:Arial,Tahoma,sans-serif;\">To <a href=\"http://bibiserv.techfak.uni-bielefeld.de/chroma/loadConfiguration\">load an existing configuration</a> for ChromA, select <em>File->Load config</em></p>");
                // sb
                // .append("<p style=\"font-family:Arial,Tahoma,sans-serif;\">To <a href=\"http://bibiserv.techfak.uni-bielefeld.de/chroma/editConfiguration\">edit an existing configuration</a> for ChromA, select <em>File->Edit config</em></p>");
                sb
                        .append("<h2 style=\"font-family:Arial,Tahoma,sans-serif;\">Running a configuration</h2>");
                sb
                        .append("<p style=\"font-family:Arial,Tahoma,sans-serif;\">After loading or creation of a new configuration, you can <a href=\"http://bibiserv.techfak.uni-bielefeld.de/chroma/runConfiguration\">run the active configuration</a> by selecting <em>Run->Run configuration</em> in the menu");
                sb
                        .append("<p style=\"font-family:Arial,Tahoma,sans-serif;\">As soon as configuration is executed, the right hand side of this window will show a progress view. After reaching 100%, the results will be presented either by opening your system's default file browser, or by showing a dialog, which gives the URL on your computer under which the results have been saved.</p>");
                ...
                LoadConfigurationAction loadConfigAction = (LoadConfigurationAction) pw
                        .getLoadConfigurationAction();
                loadConfigAction.setContainer(jsp);
                CreateConfigurationAction createConfigAction = (CreateConfigurationAction) pw
                        .getCreateConfigurationAction();
                ...
                RunConfigurationAction rca = (RunConfigurationAction) pw
                        .getRunConfigurationAction();
    ...
    public class CreateConfigurationAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            Runnable r1 = new Runnable() {
                @Override
                public void run() {
                    // How do I change the background image on the left side of
                    // the wizard dialog?
                    // Add an Image to UIManager under the key
                    // wizard.sidebar.image. Or set the system property
                    // wizard.sidebar.image to a URL that points to the image
                    // file inside a jar that is on the classpath. The image
                    // must be in a format readable by ImageIO.read().
                    Wizard wiz = WizardPage.createWizard(new Class[]{
                                FileInputOutputPane.class,
                                AnchorDefinitionPane.class,
                                PreprocessingPane.class, AlignmentPane.class,
                                VisualizationPane.class}, pw);
                    ...
                    if (cfg != null) {
                        try {
                            ...
                            JOptionPane.showMessageDialog(pw.getMessageArea()
                                    .getTopLevelAncestor(), "Loading of file "
                                    + cfg.getAbsolutePath() + " failed:\n"
     ...
     * 
     * @see
     * org.netbeans.spi.wizard.WizardPage.WizardResultProducer#cancel(java.util
     * .Map)
     */
    @Override
     ...
     * 
     * @see
     * org.netbeans.spi.wizard.WizardPage.WizardResultProducer#finish(java.util
     * .Map)
     */
    @Override
    public Object finish(Map arg0) throws WizardException {
        ...
        if (gatheredSettings != null) {
            ...
            for (Object key : gatheredSettings.keySet()) {
                ...
                if (key.equals("alignment.algorithm.distance")) {
                    ...
                    if (value.equals("Time Penalized Dot")) {
                        ...
                        cfg.setProperty((String) key,
                                "maltcms.commands.distances.ArrayCorr");
                        cfg
                                .setProperty("alignment.algorithm",
                                "maltcms.commands.distances.dtw.MZIDynamicTimeWarp");
                        ...
                        cfg.setProperty((String) key,
                                "maltcms.commands.distances.ArrayHamming");
                        cfg
                                .setProperty("alignment.algorithm",
                                "maltcms.commands.distances.dtw.MZIDynamicTimeWarp");
                        ...
                        cfg
                                .setProperty("alignment.algorithm",
                                "maltcms.commands.distances.dtw.TICDynamicTimeWarp");
                    } else {
                    ...
                    // System.out.println("Input data: "+gatheredSettings.get(key));
                    cfg
                            .setProperty("input.dataInfo", gatheredSettings
                            .get(key));
                } else if (key
                        .equals("maltcms.commands.fragments.DenseArrayProducer.maskMasses")) {
                    if (gatheredSettings
                            .get("maltcms.commands.fragments.DenseArrayProducer.maskMasses") == null) {
                        cfg
                                .setProperty(
                                "maltcms.commands.fragments.DenseArrayProducer.maskMasses",
                                "");
                    } else {
                        Object o = gatheredSettings
                                .get("maltcms.commands.fragments.DenseArrayProducer.maskMasses");
                        // System.out.println("Received object for maskMasses:"
                        // + o);
            ...
            cfg.setProperty(
                    "cross.datastructures.workflow.DefaultWorkflow.saveHTML",
                    true);
            // System.out.println(ConfigurationUtils.toString(cfg));
    ...
    public File checkOverwrite() {
        ...
        jfc
                .setDialogTitle("Select existing or create new file to save properties");
        jfc.setMultiSelectionEnabled(false);
        jfc.setFileSelectionMode(JFileChooser.FILES_ONLY);
        int ret = jfc.showSaveDialog(null);
        if (ret == JFileChooser.APPROVE_OPTION) {
            if (jfc.getSelectedFile().exists()) {
                int opRet = JOptionPane.showOptionDialog(null, "File "
                        + jfc.getSelectedFile().getAbsolutePath()
                        + "\nalready exists, overwrite?",
                        "Overwrite existing file?", JOptionPane.YES_NO_OPTION,
                        JOptionPane.QUESTION_MESSAGE, null, new String[]{
                            "Overwrite", "Cancel"}, "Cancel");
                if (opRet == JOptionPane.YES_OPTION) { // overwrite
    ...
    public void hyperlinkUpdate(HyperlinkEvent e) {
        if (e.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
            if (e.getURL().getPath().endsWith("loadConfiguration")) {
                getLoadConfigurationAction().actionPerformed(
                        new ActionEvent(this, 1, "loadConfiguration"));
            } else if (e.getURL().getPath().endsWith("createConfiguration")) {
                getCreateConfigurationAction().actionPerformed(
                        new ActionEvent(this, 1, "createConfiguration"));
            } else if (e.getURL().getPath().endsWith("editConfiguration")) {
                getEditConfigurationAction().actionPerformed(
                        new ActionEvent(this, 1, "editConfiguration"));
            } else if (e.getURL().getPath().endsWith("runConfiguration")) {
                getRunConfigurationAction().actionPerformed(
                        new ActionEvent(this, 1, "runConfiguration"));
            } else {
     ...
     * (non-Javadoc)
     * 
     * @see cross.event.IEventSource#addListener(cross.event.IListener)
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see cross.event.IEventSource#removeListener(cross.event.IListener)
     */
    @Override
