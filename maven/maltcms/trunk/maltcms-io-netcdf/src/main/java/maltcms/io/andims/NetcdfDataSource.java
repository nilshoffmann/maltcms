 ...
 * 
 * $Id: NetcdfDataSource.java 160 2010-08-31 19:55:58Z nilshoffmann $
 */
...
public class NetcdfDataSource implements IDataSource {
	...
	protected NetcdfFile locateFile(final IVariableFragment f)
		...
		NetcdfFile nf = null;
		this.log.debug("Trying to find {} in {}", f.getVarname(), f.getParent()
		        .getAbsolutePath());
		String file = "";
	...
	public ArrayList<Array> readIndexed2(final IVariableFragment f)
		...
		this.log.debug("Reading {} with index {}", f.getVarname(), index
		        .getVarname());
						...
						if (!vf.hasArray()) {
							...
							this.log
							        .debug(
							                ...
							                vf.getVarname());
							dim = cross.datastructures.tools.ArrayTools
							        .getDefaultDimensions(vf.getArray());
							final Dimension[] dimC = new Dimension[dim.length];
							int i = 0;
							for (final Dimension element : dim) {
								this.log
								        .debug("Checking Dimension {}", element);
