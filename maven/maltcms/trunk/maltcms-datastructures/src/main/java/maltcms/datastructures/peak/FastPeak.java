...
@Data
@Slf4j
public class FastPeak extends DefaultFeatureVector {
    ...
    private final Map<Peak,Integer> peakToIndex = new ConcurrentHashMap<Peak, Integer>();
    private final HashMap<String, Map<FastPeak, Double>> sims = new HashMap<String, Map<FastPeak, Double>>();
    private final HashMap<String, List<FastPeak>> sortedPeaks = new HashMap<String, List<FastPeak>>();
    private String name = "";
     ...
     * @param similarity
     */
    public void addSimilarity(final FastPeak p, final Double similarity) {
        if (this.storeOnlyBestSimilarities) {
            if (!Double.isInfinite(similarity) && !Double.isNaN(similarity)) {
                Map<FastPeak, Double> hm = null;
                FastPeak best = p;
                if (this.sims.containsKey(p.getAssociation())) {
                    ...
                    hm = new LinkedHashMap<FastPeak, Double>();
                    hm.put(p, similarity);
...
//                }
            ...
            if (!Double.isInfinite(similarity) && !Double.isNaN(similarity)) {
                ...
                if (this.sims.containsKey(p.getAssociation())) {
                    ...
                    hm = new HashMap<FastPeak, Double>();
                    hm.put(p, similarity);
                    this.sims.put(p.getAssociation(), hm);
                }
                if (this.sortedPeaks.containsKey(p.getAssociation())) {
     ...
     * @return
     */
    public List<FastPeak> getPeaksSortedBySimilarity(final String key) {
        if (this.sims.containsKey(key)) {
            List<FastPeak> peaks = null;
            if (this.sortedPeaks.containsKey(key)) {
                peaks = this.sortedPeaks.get(key);
            } else {
                final Set<Entry<FastPeak, Double>> s = this.sims.get(key).entrySet();
                final ArrayList<Entry<FastPeak, Double>> al = new ArrayList<Entry<FastPeak, Double>>();
                for (final Entry<FastPeak, Double> e : s) {
                ...
                // al.addAll(s);
                Collections.sort(al, new Comparator<Entry<FastPeak, Double>>() {
                    @Override
                    public int compare(final Entry<FastPeak, Double> o1,
                            final Entry<FastPeak, Double> o2) {
                ...
                peaks = new ArrayList<FastPeak>(al.size());
                for (final Entry<FastPeak, Double> e : al) {
                    peaks.add(e.getKey());
                }
    ...
    public FastPeak getPeakWithHighestSimilarity(final String key) {
        final List<FastPeak> l = getPeaksSortedBySimilarity(key);
        if (l.isEmpty()) {
            return null;
        }
    ...
    public Double getSimilarity(final FastPeak p) {
        if (this.sims.containsKey(p.getAssociation())) {
    ...
    public boolean isBidiBestHitFor(final FastPeak p) {
        final FastPeak pT = getPeakWithHighestSimilarity(p.getAssociation());
        final FastPeak qT = p.getPeakWithHighestSimilarity(this.getAssociation());
        if (qT == null || pT == null) {
            return false;
        }
    ...
    public void retainSimilarityRemoveRest(final FastPeak p) {
        if (this.sims.containsKey(p.getAssociation())) {
            final Map<FastPeak, Double> hm = this.sims.get(p.getAssociation());
            if (hm.containsKey(p)) {
                ...
                final ArrayList<FastPeak> al = new ArrayList<FastPeak>();
                al.add(p);
    ...
    public boolean equals(Object o) {
        if (o!=null && o instanceof FastPeak) {
