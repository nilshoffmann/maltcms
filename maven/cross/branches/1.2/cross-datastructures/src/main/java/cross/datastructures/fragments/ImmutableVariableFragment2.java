...
public class ImmutableVariableFragment2 implements IVariableFragment {
    ...
    public static IVariableFragment createCompatible(IFileFragment ff,
        ...
        ImmutableVariableFragment2 nf = new ImmutableVariableFragment2(ff, vf.
                getVarname());
    ...
    protected void adjustConsistency(Array a) {
        ...
        // } else {
        if (d != null) {
            setDimensions(cross.tools.ArrayTools.getDefaultDimensions(a));
        }
        // }
    }
    ...
    public Array getArray() {
        this.log.debug("Ranges of {}={}", getVarname(),
                Arrays.deepToString(getRange()));
        if (this.getIndex() == null) {
            try {
                final Array a = GlobalContext.getContext().getBean(IDataSourceFactory.class).
            ...
            final Array a = cross.tools.ArrayTools.glue(l);
            adjustConsistency(a);
            return a;
        }
    }
    ...
    public List<Array> getIndexedArray() {
        if (getIndex() == null) {
            ...
            if (this.useCachedList) {
                log.info("Using cached scan line list");
                return CachedList.getList(this);
            } else {
                try {
                    final List<Array> l = GlobalContext.getContext().getBean(IDataSourceFactory.class).getDataSourceFor(
