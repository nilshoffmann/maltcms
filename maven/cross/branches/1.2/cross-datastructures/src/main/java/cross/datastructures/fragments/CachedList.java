 ...
 */
@Slf4j
public class CachedList implements List<ucar.ma2.Array> {
    ...
    @Getter @Setter
    private boolean prefetchOnMiss = true;
    ...
    private void init(final int offset, final int size) {
        try {
            this.size = GlobalContext.getContext().getBean(IDataSourceFactory.class).
    ...
    private List<Array> load(final int from, final int to)
        ...
        try {
            ...
            index.setRange(r);
            // read array
            final List<Array> a = GlobalContext.getContext().getBean(IDataSourceFactory.class).
