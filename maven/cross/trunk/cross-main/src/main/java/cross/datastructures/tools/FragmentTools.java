...
public class FragmentTools {
    ...
    public static IFileFragment getLHSFile(final IFileFragment ff) {
        final String s = FragmentTools.getStringVar(ff, Factory.getInstance().getConfiguration().getString("var.reference_file",
                "reference_file"));
    ...
    public static Collection<IFileFragment> getSourceFiles(
        ...
        FragmentTools.log.debug("Trying to load {} for {}", sourceFilesVar, ff.getAbsolutePath());
        ...
        final ArrayList<IFileFragment> al = new ArrayList<IFileFragment>(c.size());
        FragmentTools.log.info("Found the following source files:");
        for (final String s : c) {
            if (new File(s).isAbsolute()) {
                al.add(Factory.getInstance().getFileFragmentFactory().create(s));
                FragmentTools.log.info("{}", al.get(al.size() - 1).getAbsolutePath());
            } else {
                try {
                    String absoluteFile = FileTools.resolveRelativeFile(new File(ff.getAbsolutePath()).getParentFile(), new File(s));
                    al.add(Factory.getInstance().getFileFragmentFactory().create(absoluteFile));
                } catch (IOException ex) {
                    java.util.logging.Logger.getLogger(FragmentTools.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
    ...
    public static String getStringVar(final IFileFragment ff,
        ...
        if (a instanceof ArrayChar.D1) {
            ...
            FragmentTools.log.warn("Received array of type {}, expected {}", a.getClass().getName(), ArrayChar.D1.class.getName());
        }
        FragmentTools.log.info("Type of received array {}", a.getClass().getName());
        return null;
    }
    ...
    public static void loadDefaultVars(final IFileFragment ff,
        ...
        final Iterator<?> iter = l.iterator();
        FragmentTools.log.debug("Loading default vars for file {}", ff.getAbsolutePath());
        while (iter.hasNext()) {
            final String var = iter.next().toString();
    ...
    public static void setLHSFile(final IFileFragment ff,
        ...
        FragmentTools.createString(ff, Factory.getInstance().getConfiguration().getString("var.reference_file", "reference_file"), lhs.getAbsolutePath());
    }

    public static void setRHSFile(final IFileFragment ff,
            final IFileFragment rhs) {
        FragmentTools.createString(ff, Factory.getInstance().getConfiguration().getString("var.query_file", "query_file"), rhs.getAbsolutePath());
    }
}
