            ...
            for (final List<Point> l : bidiBestHitList) {
                ...
                for (final Point p : l) {
                    if (p.x != -1) {
                        ...
                        // + "(" + p.x + ")");
                        table.get(p.y + 1).add(
                                this.formatter.format(peaklist.get(p.y).get(p.x).getPeakArea().getAreaIntensity()));
//                                                                p.x + "");
            ...
            csvw.writeTableByCols(this.iworkflow.getOutputDirectory(this).getAbsolutePath(), "bidibestHits.csv", table,
        ...
        if (bidiBestHitList.size() > 0) {
            ...
            for (final List<Point> l : bidiBestHitList) {
                for (final Point p : l) {
                    if (p.x != -1) {
                        table.get(p.y).add(
                                formatter.format(peaklist.get(p.y).get(p.x).getRetentionTime()));
                    } else {
                        // table.get(p.y + 1).add("-(-)");
            ...
            final CSVWriter csvw = new CSVWriter();
            csvw.setIWorkflow(this.iworkflow);
            csvw.writeTableByCols(this.iworkflow.getOutputDirectory(this).getAbsolutePath(), "multiple-alignmentRT.csv", table,
                    WorkflowSlot.PEAKFINDING);
        } else {
            this.log.error("Bidirectional best hit list ist empty");
        }
    ...
    public void exportDetailedBBHInformation(
                    ...
                    for (Datatypes t : Datatypes.values()) {
                        switch (t) {
                            ...
                            case PeakArea:
                                ...
                                metaTable.get(i).add(
                                        this.formatter.format(peak.getPeakArea().getAreaIntensity()));
                                break;
                            case LogPeakArea:
                                metaTable.get(i).add(
                                        this.formatter.format(Math.log(peak.getPeakArea().getAreaIntensity())));
                                break;
                            case RetT1:
                                metaTable.get(i).add(
                                        this.formatter.format(peak.getFirstRetTime()));
                                break;
                            case RetT2:
                                metaTable.get(i).add(
                                        this.formatter.format(peak.getSecondRetTime()));
                                break;
                            default:
                                break;
                        }
                            ...
                            switch (t) {
                                case PeakAreaRatio:
                                    v = peak1.getPeakArea().getAreaIntensity()
                                            / peak2.getPeakArea().getAreaIntensity();
        ...
        final CSVWriter csvw = new CSVWriter();
        csvw.setIWorkflow(this.iworkflow);
        csvw.writeTableByCols(this.iworkflow.getOutputDirectory(this).getAbsolutePath(), oFilename, metaTable,
                WorkflowSlot.PEAKFINDING);
    }
    ...
    public void exportPeakOccurrenceMap(
        ...
        for (int i = 0; i < bidiBestHitList.size(); i++) {
            for (Point p : bidiBestHitList.get(i)) {
                if (p.x != -1) {
                    ...
                    row.add(this.formatter.format(peak.getPeakArea().getAreaIntensity()));
                    row.add(this.formatter.format(peak.getPeakArea().getSeedIntensity()));
                    metaTable.add(row);
                } else {
                    // no peak match
                }
        ...
        final CSVWriter csvw = new CSVWriter();
        csvw.setIWorkflow(this.iworkflow);
        csvw.writeTableByRows(this.iworkflow.getOutputDirectory(this).getAbsolutePath(), oFilename, metaTable,
                WorkflowSlot.PEAKFINDING);
    }
    ...
    public void exportDetailedPeakInformation(final String name,
        ...
        for (final Peak2D p : ps) {
            ...
            for (final Entry<Integer, Double> e : p.getPeakArea().getAreaIntensities().entrySet()) {
                row.set(masses.indexOf(e.getKey()) + 1, this.formatter.format(e.getValue())
                        + "");
            }
            printTable.add(row);
        }
        ...
        final CSVWriter csvw = new CSVWriter();
        csvw.setIWorkflow(this.iworkflow);
        csvw.writeTableByRows(this.iworkflow.getOutputDirectory(this).getAbsolutePath(), name + "_peakAreas.csv", printTable,
                WorkflowSlot.PEAKFINDING);
    }
...
//			peaklist.add(peak);
        ...
        final CSVWriter csvw = new CSVWriter();
        csvw.setIWorkflow(this.iworkflow);
        csvw.writeTableByRows(this.iworkflow.getOutputDirectory(this).getAbsolutePath(), name + "_peaklist.csv", table,
                WorkflowSlot.PEAKFINDING);
    }
    ...
    public void exportPeaksToMSP(final String name, final List<Peak2D> ps,
        ...
        try {
            ...
            for (Peak2D p : ps) {
                ...
                final ArrayInt.D1 mz = new ArrayInt.D1(denseIntensities.getShape()[0]);
                ...
                if (t != null && t.getFirst() != null && t.getSecond() != null) {
                    int mw = (int) MaltcmsTools.getMaxMass(t.getFirst(), t.getSecond());
                    ArrayDouble.D1 masses = new ArrayDouble.D1(t.getFirst().getShape()[0]);
                    ArrayInt.D1 intensities = new ArrayInt.D1(t.getSecond().getShape()[0]);
                    MAMath.copyDouble(masses, t.getFirst());
                    MAMath.copyInt(intensities, t.getSecond());
                    Metabolite2D m = new Metabolite2D(pname, pname,
                            "CHROMA4D-ID", cnt, "", "", this.iworkflow.getStartupDate().toString(), 0, p.getFirstRetTime(), "min", mw, "", p.getName(),
    ...
    public void exportPeakNames(final List<Peak2D> peaklist, final String chomatogramName) {
        ...
        final CSVWriter csvw = new CSVWriter();
        csvw.setIWorkflow(this.iworkflow);
        csvw.writeTableByRows(this.iworkflow.getOutputDirectory(this).getAbsolutePath(), chomatogramName + "_peakIdentifications", metaTable,
                WorkflowSlot.PEAKFINDING);
    }
