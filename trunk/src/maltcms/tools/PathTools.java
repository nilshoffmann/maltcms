...
public class PathTools implements IConfigurable {
	...
	public List<Tuple2DI> makeMap(final IFileFragment parent,
		...
		// if(traceForConvex) {
		// return makeMapForConvex(parent, cdist, d, minimize,
		// cumulativeDistanceClass);
		// }
	...
	public Tuple2DI nextStep2(final List<Tuple2DI> map1,
		...
		// if (minimize) {
		// return minimizeSteps(map1, arr, d, a, b, cumulativeDistanceClass,
		// sb);
		// } else {
		// return maximizeSteps(map1, arr, d, a, b, cumulativeDistanceClass,
		// sb);
		// }
	}
	...
	public Tuple2DI traceback(final List<Tuple2DI> map1,
		...
		if (a == 0) {// first row
			...
			// weight_sum.set(weight_sum.get()+cumulativeDistanceClass.getExpansionWeight());
		...
		if (b == 0) {// first column
			PathTools.log.debug("({},{})", a, b);
			// weight_sum.set(weight_sum.get()+cumulativeDistanceClass.getCompressionWeight());
		...
		try {// get values of predecessors
			// i = cumulativeDistanceClass.cumDist(a-1, b, arr, d.get(a-1, b));
			// j = cumulativeDistanceClass.cumDist(a-1, b-1, arr, d.get(a-1,
			// b-1));
			// k = cumulativeDistanceClass.cumDist(a, b-1, arr, d.get(a, b-1));
			i = arr.get(a - 1, b);
			j = arr.get(a - 1, b - 1);
			k = arr.get(a, b - 1);
			// i = cumulativeDistanceClass.cumDist(a-1, b, arr,
			// d.get(a,b));//arr.get(a-1, b);//
			// cumulativeDistanceClass.cumDist(a, y, arr,
			// // d.get(a,b));//(arr.getDouble(ind.set(a,
			// // y)));// map b and b-1
			// // values to a
			// j = cumulativeDistanceClass.cumDist(a-1, b-1, arr,
			// d.get(a,b));//arr.get(a-1, b-1);//
			// cumulativeDistanceClass.cumDist(x, y, arr,
			// // d.get(a,b));//(arr.getDouble(ind.set(x,
			// // y)));// map a to b
			// k = cumulativeDistanceClass.cumDist(a, b-1, arr,
			// d.get(a,b));//arr.get(a, b-1);//
			// cumulativeDistanceClass.cumDist(x, b, arr,
			// // d.get(a,b));//(arr.getDouble(ind.set(x,
			// // b)));// map a and a-1
			// // values to b
			PathTools.log.debug(
		...
		if ((val == j)) {
			// weight_sum.set(weight_sum.get()+cumulativeDistanceClass.getDiagonalWeight());
