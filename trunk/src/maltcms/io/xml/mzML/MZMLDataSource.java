...
public class MZMLDataSource implements IDataSource {
	...
	private MzMLUnmarshaller getUnmarshaller(final IFileFragment ff) {
		...
		MzMLUnmarshaller um = new MzMLUnmarshaller(new File(ff
	...
	private Array getMassValues(final Spectrum s) {
		Number[] n = s.getBinaryDataArrayList().getBinaryDataArray().get(0)
	...
	private Array getIntensityValues(final Spectrum s) {
		Number[] n = s.getBinaryDataArrayList().getBinaryDataArray().get(1)
		        .getBinaryDataAsNumberArray();
	...
	private String getRTUnit(final Spectrum s) {
		...
		try {
			CVParam rtp = findParam(s.getScanList().getScan().get(0)
			        .getCvParam(), "MS:1000016");
	...
	public ArrayList<Array> readIndexed(final IVariableFragment f)
		...
		if (f.getVarname().equals(this.mass_values)) {
			...
			final Run run = getRun(getUnmarshaller(f.getParent()), f
			        .getParent());
			int start = 0;
	...
	private Array readTotalIntensitiesArray(final IVariableFragment var,
		...
		BinaryDataArray timeArray = c.getBinaryDataArrayList()
		        .getBinaryDataArray().get(0);
		BinaryDataArray intensitiesArray = c.getBinaryDataArrayList()
		        .getBinaryDataArray().get(1);
