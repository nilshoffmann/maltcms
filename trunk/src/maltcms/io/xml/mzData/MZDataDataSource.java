...
public class MZDataDataSource implements IDataSource {
	...
	private Tuple2D<Double, Double> getMinMaxMassRange(final MzData mp,
	        final int scan) {
		final SpectrumSettingsType.SpectrumInstrument si = mp.spectrumList.spectrum
	...
	private Array getPeakListArray(final PeakListBinaryType blbt) {
		...
		Array a = null;
		if (prec == 32) {
			final float[] f = Base64Util.byteArrayToFloatArray(b, endian
			...
			final double[] d = Base64Util.byteArrayToDoubleArray(b, endian
			        .equals("little") ? false : true, length);
	...
	private double getRT(final MzData mz, final int j) {
		...
		final List<Object> l = s.getSpectrumDesc().getSpectrumSettings()
	...
	public ArrayList<Array> readIndexed(final IVariableFragment f)
		...
		if (f.getVarname().equals(this.intensity_values)) {
			...
			for (int i = 0; i < len; i++) {
				al
				        .add(getPeakListArray(l.get(i + start)
				                .getIntenArrayBinary()));
			}
			return al;
		}
	...
	private Array readTotalIntensitiesArray(final IVariableFragment var,
		...
		for (int i = 0; i < getScanCount(mp); i++) {
			...
			final double v = ArrayTools.integrate(a);
			d[i] = v;
		}
