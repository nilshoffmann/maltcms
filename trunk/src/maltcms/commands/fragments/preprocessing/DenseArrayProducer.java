	...
	private void maskMasses(final Tuple2D<Double, Double> minmax,
		...
		if ((this.maskedMasses != null) && !this.maskedMasses.isEmpty()) {
			...
			final IVariableFragment total_intens = f
			        .hasChild(this.total_intensity) ? f
			        .getChild(this.total_intensity) : new VariableFragment(f,
			...
			for (final Array a : intens) {
				...
				for (int i = 0; i < a.getShape()[0]; i++) {
					if (selector.get(i) == 1.0d) {
						...
						aidx.set(i);
						tic += a.getDouble(aidx);
					}

				}
				tanidx.set(scan);
				tan.setDouble(tanidx, tic - accum);
				filtered.add(a);
				scan++;
			}
	...
	private void normalizeScans(final IFileFragment f) {
		if (this.normalizeScans || this.normalizeMeanVariance) {
			...
			if (this.normalizeMeanVariance) {
				this.log
				        .info("Normalizing by subtracting mean and dividing by variance!");
