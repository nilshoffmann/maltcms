...
public class ChromatogramWarp2 extends AFragmentCommand {
	...
	@Configurable(name = "var.anchors.retention_index_names", type = String.class)
	...
	private AlignmentTable buildTableFromFragment(IFileFragment f) {
		try {
			IVariableFragment ma = f
			        .getChild(this.multipleAlignmentVariableName);
			ArrayChar.D3 maa = (ArrayChar.D3) ma.getArray();
			IVariableFragment maNames = f
			        .getChild(this.multipleAlignmentNamesVariableName);
	...
	public TupleND<IFileFragment> apply(final TupleND<IFileFragment> t) {
		...
		for (int i = 0; i < wt.size(); i++) {
			...
			this.log.info("Processing {}/{}", (i + 1), wt.size());
			if (!StringTools.removeFileExt(queryTarget.getName()).equals(
			        refOrig.getName())) {
				this.log.info("Warping {} to {}", querySource.getName(),
				        refOrig.getName());
				IFileFragment res = warp(refOrig, querySource, queryTarget, at
				        .getPathFor(refOrig, queryTarget), true, getIWorkflow());
				res.save();
	...
	class AlignmentTable {
		...
		List<Tuple2DI> getPathFor(IFileFragment ref, IFileFragment query) {
			List<int[]> refIdx = columnMap.get(StringTools.removeFileExt(ref
			        .getName()));
			List<int[]> queryIdx = columnMap.get(StringTools
