...
public class TICHeatmapCoplot extends AFragmentCommand {
	...
	public void configure(Configuration cfg) {
		...
		this.scanAcquisitionTimeVar = cfg
		        .getString("var.scan_acquisition_time");
	}
	...
	protected File drawTICSJFreeChart(final File outputDir,
		...
		for (IFileFragment f : t) {
			length = Math.max(length,
			        f.getChild(satVar).getArray().getShape()[0]);
		}
		double[][] data = new double[3][length * t.size()];
		int k = 0;
		for (IFileFragment f : t) {
			...
			for (int i = 0; i < tic.length; i++) {
				...
				data[2][k] = tic[i];
				min = Math.min(min, tic[i]);
				max = Math.max(max, tic[i]);
				k++;
		...
		GradientPaintScale ps = new GradientPaintScale(ImageTools
		        .createSampleTable(256), min, max, ImageTools
		        .rampToColorArray(new ColorRampReader()
		                .readColorRamp(this.colorRampLocation)));
