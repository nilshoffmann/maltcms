...
public class Default2DVarLoader extends AFragmentCommand {
	...
	public void configure(final Configuration cfg) {
		...
		this.modulationTime = cfg
		        .getDouble("var.modulation_time.default", 5.0d);
	}
	...
	private IVariableFragment create1DTic(final IFileFragment source,
		...
		final List<Array> tic = buildIndexedArray(scanspermpodualtion, source
		        .getChild(this.totalIntensityVar).getArray());
		final List<Array> sat = buildIndexedArray(scanspermpodualtion, source
		...
		tic1dvar.setDimensions(new Dimension[] { new Dimension("scan_number2d",
		        tic.size(), true) });
		final IVariableFragment sat1dvar = new VariableFragment(parent,
		        this.scanAcquisitionTime1dVar);
		sat1dvar.setArray(sat1d);
		sat1dvar.setDimensions(new Dimension[] { new Dimension("scan_number2d",
		        tic.size(), true) });
		return tic1dvar;
	}
	...
	private IVariableFragment createScanRate(final IFileFragment source,
		...
		final IVariableFragment scanratevar = new VariableFragment(parent,
		        this.scanRateVar);
		final Array scanRateArray = Array
		        .factory(Double.class, new int[] { 1 });
	...
	private IVariableFragment createSecondColumnIndex(
		...
		index2dvar.setDimensions(new Dimension[] { new Dimension(
		        "scan_number2d", modulationCnt, true) });
		return index2dvar;
	}
	...
	private IVariableFragment createSecondColumnTime(
		...
		final Array secondRetTimeArray = Array.factory(Double.class, source
		        .getChild(this.totalIntensityVar).getArray().getShape());
		...
		while (ticiter.hasNext() && timeiter.hasNext()) {
			timeiter
			        .setDoubleNext(((c % scanspermodulation) * this.scanDuration)
			                + this.scanDuration);
			ticiter.next();
			c++;
		}
