...
public class MeanVarProducer extends AFragmentCommand {
	...
	public TupleND<IFileFragment> apply(final TupleND<IFileFragment> t) {
		...
		for (final IFileFragment ff : t) {
			this.log.info("Computing mean and std for {}", StringTools
			...
			if (this.minStandardDeviationQuantil >= 0.0d) {
				this.minStandardDeviation = ArrayTools2.getQuantilValue(ff, sd,
				        new double[] { this.minStandardDeviationQuantil },
				        true, this)[0];
				this.log.info(
			...
			final IVariableFragment meanV = new VariableFragment(fret,
			        this.meanMSIntensityVar);
			meanV.setArray(mean);
			final IVariableFragment varV = new VariableFragment(fret,
			        this.varMSIntensityVar);
			varV.setArray(var);
			final IVariableFragment sdV = new VariableFragment(fret,
			        this.sdMSIntensityVar);
			sdV.setArray(sd);
			final IVariableFragment intV = new VariableFragment(fret,
			        this.vIntensityVar);
			intV.setArray(intensities.getFirst());
			final IVariableFragment int1DV = new VariableFragment(fret,
	...
	private Tuple2D<ArrayDouble.D1, ArrayDouble.D1> computeNewIntensity(
		...
		final ArrayDouble.D1 intensities = new ArrayDouble.D1(slc
		        .getLastIndex());
		final ArrayDouble.D1 intensities1D = new ArrayDouble.D1(slc
		        .getScanLineCount());
	...
	public void configure(final Configuration cfg) {
		...
		if (this.minStandardDeviation >= 0) {
			...
			this.minStandardDeviationQuantil = cfg.getDouble(this.getClass()
			        .getName()
	...
	private Tuple2D<ArrayDouble.D1, ArrayDouble.D1> calculateMeanVarSparse(
		...
		for (int i = 0; i < slc.getScanLineCount(); i++) {
			...
			final List<Tuple2D<Array, Array>> scanline = slc
			        .getScanlineSparseMS(i);
			for (final Tuple2D<Array, Array> ms : scanline) {
				final IndexIterator massIter = ms.getFirst().getIndexIterator();
				final IndexIterator intenIter = ms.getSecond()
				        .getIndexIterator();
