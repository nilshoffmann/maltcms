...
public class MeanVarVis extends AFragmentCommand {
	...
	public TupleND<IFileFragment> apply(final TupleND<IFileFragment> t) {
		for (final IFileFragment ff : t) {
			...
			createChartXY(mean, "Mean mz bin signal", StringTools
			        .removeFileExt(ff.getName())
			        + "_mean", this.useLogScale, ff);
			createChartXY(var, "Variance in mz bins", StringTools
			        .removeFileExt(ff.getName())
			        + "_variance", this.useLogScale, ff);
			createChartXY(sd, "Standard deviation of mz bins", StringTools
			        .removeFileExt(ff.getName())
			        + "_standardDeviation", this.useLogScale, ff);
			ArrayTools2.getQuantilValue(ff, sd, new double[] { 0.001, 0.002,
			        0.005, 0.01, 0.02, 0.05, 0.1, 0.2, 0.5 }, true, this);

			// 1D TIC VIS
			final Array tic1do = ff.getChild(this.totalIntensity1dVar)
			        .getArray();
			Array tic1dsd = ff.getChild(this.vtotalIntensity1DVar).getArray();
			final Array rettime = ff.getChild(this.scanAcquisitionTime1dVar)
			        .getArray();
	...
	private void createChartXY(final Array array, final String name,
		...
		final DefaultWorkflowResult dwr1 = new DefaultWorkflowResult(pl
		        .getFile(), this, this.getWorkflowSlot(), resource);
	...
	private void createImage(final String filename, final String title,
		...
		final BufferedImage bi2 = maltcms.tools.ImageTools.fullSpectrum(title,
		        ...
		        lowThreshold);
		ImageTools.saveImage(bi2, filename, this.format, getIWorkflow()
