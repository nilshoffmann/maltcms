...
public class Default2DTWVisualizer implements IVisualization {
	...
	public BufferedImage createImage(List<Array> scanlinesi,
		...
		return ci(outputintensities, outputintensitiescounter,
		        (int) (scanlinesi.get(0).getSize()), sb.getFirst(), sb
		                .getSecond().getFirst(), sb.getSecond().getSecond());
	}
	...
	protected Tuple2D<double[], Tuple2D<double[], double[]>> getSampleAndBreakpointTable(
		...
		final Array scanlinesiC = cross.tools.ArrayTools.glue(scanlinesi);
		final Array scanlinesjC = cross.tools.ArrayTools.glue(scanlinesj);
	...
	public void configure(final Configuration cfg) {
		...
		this.holdj = cfg
		        .getBoolean(this.getClass().getName() + ".holdj", false);
	...
	public JFreeChart createChart(final String filename,
		...
		if (this.holdi && !this.holdj) {
			...
			domain.setRange(irettime.get(0), irettime
			        .get(irettime.getShape()[0] - 1));
			...
			domain.setRange(jrettime.get(0), jrettime
			        .get(jrettime.getShape()[0] - 1));
		} else {
			log
			        .info("Using neither scan acquisition time from the first nor from the second chromatogram. Using the warp path index for the domain axis.");
	...
	public JFreeChart addPeakMarker(final JFreeChart chart,
		...
		while (iterident.hasNext()) {
			...
			addMarker(plot, firstColumnTime.get(x + 1), secondColumnTime
			        .get(y + 1), true, refid++);
		}
