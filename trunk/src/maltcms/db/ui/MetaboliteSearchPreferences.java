...
public class MetaboliteSearchPreferences extends JFrame {
	 ...
	 * @return msp
	 */
	public static MetaboliteSearchPreferences getInstance(MetaboliteViewModel mvm){
		if(msp == null){
	 ...
	 * @param tfHashSize
	 */
	private HashMap<Integer, JTextField> addTooltipsToTextFields(HashMap<Integer, JTextField> textFieldsHash, int tfHashSize) {
		
		String stringHelp = "<html>This field takes all variants of strings to filter"+"<ul><li>Enable substring filtering by adding a '<' like: fie<<li></html>"; 
		String intHelp = "<html>This field takes two variants:"+"<ul><li>A single whole number (Integer) like: 1<li>" +
				"A range of integers like: 10,20</ul></html>";
		String doubleHelp = "<html>This field takes two variants:"+"<ul><li>A single floating-point number (Double) like: 1.54<li>" +
	...
	private JButton getSubmitFilterButton(){
		if(this.submitFilterButton == null){
			...
			ActionListener al = new FilterButtonActionListener(mvm, error, textFields, this);
		...
		return this.submitFilterButton;
			...
			idTF);				//string
			massWeightTF); 		//int	--
			massSpectrumTF);//cross.datastructures.tuple.Tuple2D<ucar.ma2.ArrayDouble$D1, ucar.ma2.ArrayInt$D1>
			maxIntensityTF);	//double	--
			maxIntNormalizedTF);//double	--
			maxMassTF);			//double	--
			minIntensityTF);	//double	--
			minIntNormalizedTF);//double	--
			minMassTF);			//double	--
			nameTF);			//string
			retentionIndexTF);	//double	--
			retentionTimeTF);	//double	--
			retentionTimeUnitTF);//string
			spTF);				//string
			scanIndexTF);		//int	--
			shortNameTF);		//string
			*/
