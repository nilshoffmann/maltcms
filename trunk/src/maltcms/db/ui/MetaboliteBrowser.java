...
public final class MetaboliteBrowser extends JFrame {
	...
	public static final ObjectContainer getObjectContainer(Credentials c)
		...
		if (f.exists()) {
			...
			oc = Db4o.openClient(url.getHost(), url.getPort(), url
			        .getUserInfo(), c.getPassword());
		}
		return oc;
	}
	...
	public static void main(String[] args) {
		...
		Runnable r = new Runnable() {
			...
			public void run() {
				...
				JButton jb = new JButton(new AbstractAction("Open Connection") {
						...
						Runnable r = new Runnable() {
							...
							public void run() {
								...
								if (c.getLocation() == null) {
									System.err
									        .println("No db selected, exiting!");
									System.exit(1);
								}
								ObjectContainer oc;
								try {
									oc = MetaboliteBrowser
									        .getObjectContainer(c);
									// opening JFrame
