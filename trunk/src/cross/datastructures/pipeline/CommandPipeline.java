...
public class CommandPipeline implements ICommandSequence {
	...
	private HashMap<AFragmentCommand, String> cmdToConfig = new HashMap<AFragmentCommand, String>();
	...
	protected ArrayList<AFragmentCommand> addToList(
		...
		for (final Tuple2D<String, String> s : commands) {
			...
			final AFragmentCommand cmd = loadCommand(s.getFirst(), s
			        .getSecond());
	...
	@Override
	public void init() {
		final List<AFragmentCommand> cmds = addToList(this.tmp, this.pipeline);
	...
	protected AFragmentCommand loadCommand(final String clsname,
		...
		final AFragmentCommand clazz = Factory.getInstance().getObjectFactory()
		        .instantiate(clsname, AFragmentCommand.class,
		                propertiesFileName);
	...
	public void appendXML(Element e) {
		...
		for (final AFragmentCommand wr : getCommands()) {
