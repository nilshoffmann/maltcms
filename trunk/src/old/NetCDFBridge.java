...
public class NetCDFBridge implements Runnable {
	...
	public static void main(final String[] args) {
		...
		NetcdfDataset nds;
		try {
			nds = NetcdfDataset.acquireDataset(NetCDFBridge.FILELOC1, null);
				...
				try {
					...
					for (long i = start; i < size; i++) {
						...
						if ((i1 >= 0) && (i2 >= 0)) {
							...
							final IFileFragment di1 = Factory.getInstance()
							        .getFileFragmentFactory().fromString(
							                NetCDFBridge.FILELOC1 + ">" + s1
							                        + range);
							final IFileFragment di2 = Factory.getInstance()
							        .getFileFragmentFactory().fromString(
